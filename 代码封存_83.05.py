...
class DualBackboneModel(nn.Module):
    def __init__(self, num_classes=6):
        super().__init__()
        # Swin 分支（正常）
        self.swin = swin_base_patch4_window7_224_in22k(pretrained=True, num_classes=0)

        # ResNet 分支（手动移除分类层）
        self.resnet = Resnet50(pretrained=True, num_classes=1000)  # 先加载完整模型
        self.resnet.fc = nn.Identity()  # 移除分类层
        self.resnet.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 显式保留全局平均池化

        # 投影层
        self.swin_proj = nn.Linear(1024, 512)
        self.resnet_proj = nn.Linear(2048, 512)
        self.classifier = nn.Linear(1024, num_classes)

    def execute(self, x):
        # Swin 分支
        x_swin = self.swin(x)  # [B,1024]

        # ResNet 分支
        x_resized = nn.interpolate(x, size=(224, 224), mode='bilinear')
        x_resnet = self.resnet(x_resized)  # [B,2048]

        # 投影和融合
        x_swin = self.swin_proj(x_swin)
        x_resnet = self.resnet_proj(x_resnet)
        x_fused = jt.concat([x_swin, x_resnet], dim=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelroot', type=str, default=os.path.join(project_root, 'code/JT/baseline/model_save_3'))
    parser.add_argument('--testonly', action='store_true', default=False)
    parser.add_argument('--loadfrom', type=str, default=os.path.join(project_root, 'checkpoints/checkpoint3.pkl'))
    parser.add_argument('--result_path', type=str, default=os.path.join(project_root, 'code/JT/baseline/result_3.txt'))
    args = parser.parse_args()

    model = DualBackboneModel(num_classes=6)
